...
For full local request/response tracing, start it with `--debug`:

```bash
uv run usai-claude-proxy --debug
```

Debug mode logs incoming Anthropic Messages payloads, translated Chat
Completions payloads, upstream responses or stream chunks, and emitted Anthropic
SSE events to stderr. It includes message content and tool arguments, so use it
only when the log destination is appropriate for that data.
